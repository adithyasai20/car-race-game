...
const N=8;
const cars=generateCars(1, "KEYS").concat(generateCars(N-1, "AI"));
const myCar=cars[0];
...
for(let i = 0;i<cars.length;i++){
    ...
    ele.id = "stat_"+i;
    ele.innerText = i;
    ele.style.color = cars[i].color;
    ele.classList.add("stat");
    statistics.appendChild(ele);
}
...
function updateCarProgress(car){
    ...
    if(!car.finishTime){
        ...
        car.progress = car.progress/totalDistance;
        if(car.progress >=1){
            car.progress = 1;
