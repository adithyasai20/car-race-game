class Camera{
    ...
    #extrude(polys, height = 10){
        const extrudedPolys = [];
        for(const poly of polys){
            const ceiling = new Polygon( poly.points.map((p) => new Point(p.x, p.y, -height)) );
    ...
    #getPolys(world){
        ...
        const roadPolys = this.#extrude(this.#filter(world.corridor.borders.map((s) => new Polygon([s.p1, s.p2]))), 10);
    ...
    render(ctx, world){
        ...
        const projectedPolys = polys.map((poly) => new Polygon(poly.points.map((p) => this.#projectPoint(ctx, p))));
